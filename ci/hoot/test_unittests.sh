...
cd build-ci/hootcore-$BUILD_TARGET
...
if [ "$DIRECT_WINE_EXEC_TESTS" = "true" ]; then
  ...
  wine ./src/test/test_hoot.exe
else
  if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
    ${TEST_RUNNER_ENV} ./src/test/test_hoot --catch_system_errors=no -l test_suite
  else
