# Fuzzing Hootchain Core using libFuzzer
...
Every single pull request submitted against the Hootchain Core repo is automatically tested against all inputs in the [`bitcoin-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Hootchain Core more robust.
...
# Fuzzing Hootchain Core using afl++
...
# Fuzzing Hootchain Core using Honggfuzz
...
## Fuzzing the Hootchain Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Hootchain
Core without having to write any custom fuzzing harness. The `hootd` server
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/hootd -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
