...
The Hootchain Core daemon can be configured to act as a trusted "border
router", implementing the hoot wire protocol and relay, making
...
The ZeroMQ feature in Hootchain Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building hootd:
...
For instance:
    ...
    $ hootd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/hootd.tx.raw \
...
These options can also be provided in hoot.conf.
...
From the perspective of hootd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into hootd directly. Furthermore, no information is
...
using. Hootchaind appends an up-counting sequence number to each
