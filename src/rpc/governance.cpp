...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 HOOT irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on hootcentral etc. and do not save them...
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* Hootchain features */
    { "hoot",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "hoot",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "hoot",               "gobject",                &gobject,                {} },
    { "hoot",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
